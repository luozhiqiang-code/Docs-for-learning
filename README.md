...
消息队列：这里面包含了很多内部消息类型，如用户交互事件（鼠标滚动、点击、移动）、文件读写、WebSocket，与页面渲染相关的事件，如 JavaScript 执行、解析 DOM、样式计算、布局计算、CSS 动画等。

IO线程：负责接受其他进程传来的消息与渲染主线程通信，产生新的任务，添加到消息队列中。

定时器：为了支持定时器的实现，浏览器增加了延时队列。在每次执行完消息队列的一个宏任务后会判断延时任务是否需要执行，然后执行延时任务。然后循环整个过程。
...
比如某论坛上有系统漏洞，可以从发表评论里面插入JavaScript标签，那么黑客可以插入一段脚本，通过document.cookie获取用户脚本然后发送到自己的服务器上。比如在有漏洞的邮箱里面植入脚本，想后端发送删除邮件的请求，那么只要一打开垃圾邮件就会导致邮件被删除。
...
4. 对输入进行过滤，包括URL、查询关键字、post数据、input输入内容。
...
举例：邮箱中的钓鱼邮件，qq邮箱有很多钓鱼邮件。如果打开了邮件中的网页，就可以通过img标签或者表单向已经腾讯后台发送删除邮件请求，然后邮件就被删除了。如果登录了淘宝，淘宝又安全漏洞，就可以想淘宝后端发送转账请求。

防御方式：

1. get请求值请求数据，不修改数据。那么就可以避免那些通过带src属性的标签伪造的请求。
